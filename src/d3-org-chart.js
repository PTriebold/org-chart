...
export class OrgChart {
        ...
        const attrs = {
            ...
            */
            ...
            pagingButton: (d, i, arr, state) => {
                ...
                const diff =
                    d.parent.data._directSubordinatesPaging - currentIndex;
            ...
            hdiagonal: function (s, t, m) {
                ...
                let r =
                    Math.abs(ex - x) / 2 < rdef ? Math.abs(ex - x) / 2 : rdef;
            ...
            layoutBindings: {
                left: {
                    ...
                    linkCompactYStart: (node) =>
                        node.y +
                        (node.compactEven ? node.height / 2 : -node.height / 2),
                    ...
                    nodeFlexSize: ({
                        ...
                        if (state.compact && node.flexCompactDim) {
                            const result = [
                                node.flexCompactDim[0],
                                node.flexCompactDim[1],
                            ];
                            return result;
                        }
                        return [
                            height + siblingsMargin,
                            width + childrenMargin,
                        ];
                    },
                ...
                top: {
                    ...
                    linkCompactXStart: (node) =>
                        node.x +
                        (node.compactEven ? node.width / 2 : -node.width / 2),
    ...
    render() {
        ...
        attrs.flexTreeLayout = flextree({
            ...
            nodeA.parent == nodeB.parent
                ? 0
                : attrs.neighbourMargin(nodeA, nodeB),
        );
    ...
    addNode(obj) {
        ...
        const parentFound = descendants.filter(
            ({ data }) =>
                attrs.nodeId(data).toString() ===
                attrs.parentNodeId(obj).toString(),
        )[0];
        if (nodeFound) {
            console.log(
                `ORG CHART - ADD - Node with id "${attrs.nodeId(obj)}" already exists in tree`,
            );
            return this;
        }
        ...
        root.eachBefore((node) => {
            if (node.children && node.children.length > 1) {
                ...
                const evenMaxColumnDimension = d3.max(
                    ...
                    attrs.layoutBindings[attrs.layout].compactDimension
                        .sizeColumn,
                );
    ...
    update({ x0, y0, x = 0, y = 0, width, height }) {
            ...
            .attr("d", (d) => {
                const n =
                    attrs.compact && d.flexCompactDim
                        ? {
                              x: attrs.layoutBindings[
                                  attrs.layout
                              ].compactLinkMidX(d, attrs),
                              y: attrs.layoutBindings[
                                  attrs.layout
                              ].compactLinkMidY(d, attrs),
                          }
                        : {
        ...
        const maxDepthNode = nodeExitTransition
            .data()
            .reduce((a, b) => (a.depth < b.depth ? a : b), { depth: Infinity });
            ...
            if (node.children) {
                node.children.forEach((child, j) => {
                    ...
                    if (
                        ...
                        while (
                            localNode &&
                            (hiddenNodesMap[localNode.id] ||
                                localNode.data._pagingButton)
                        ) {
    ...
    zoomTreeBounds({
        ...
        y0,
        y1,
        params = { animate: true, scale: true, onCompleted: () => {} },
    }) {
        const {
            centerG,
            svgWidth: w,
    ...
    setExpanded(id, expandedFlag = true) {
        ...
        if (!node) {
            console.log(
                `ORG CHART - ${expandedFlag ? "EXPAND" : "COLLAPSE"} - Node with id (${id})  not found in the tree`,
            );
            return this;
        }
    ...
    setUpToTheRootHighlighted(nodeId) {
        ...
        node.ancestors().forEach(
            (d) => (d.data._upToTheRootHighlighted = true),
        );
        return this;
    }
